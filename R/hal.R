...
should_subsample_stage2_cv <- function(fit_control, adaptive_screen, x_basis, stage2_foldid) {
...
refit_augmented_basis_with_local_lambda_path <- function(fit_control, x_basis,
  ...
  if (should_subsample_stage2_cv(
    fit_control = fit_control,
    adaptive_screen = adaptive_screen,
    x_basis = x_basis,
    stage2_foldid = stage2_foldid
  )) {
  ...
  if (should_use_approx_gaussian_backend(
    ...
    if (screen_target < penalized_count) {
      ...
      approx_fit_meta <- list(
        ...
        stage2_subsample_n = refined_fit$stage2_subsample_n
      )
    } else {
      approx_fit_meta <- list(
        used = FALSE,
